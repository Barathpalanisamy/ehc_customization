...
class ReceivablePayableReport:
	...
	def get_cost_center_conditions(self):
		cost_centers = self.filters.get("cost_center", [])
		cost_center_list = []
		for cost_center in cost_centers:
			lft, rgt = frappe.db.get_value("Cost Center", cost_center, ["lft", "rgt"])
			relevant_centers = frappe.get_list("Cost Center", filters={"lft": [">=", lft], "rgt": ["<=", rgt]})
			if relevant_centers:
				cost_center_list.extend([center.name for center in relevant_centers])
