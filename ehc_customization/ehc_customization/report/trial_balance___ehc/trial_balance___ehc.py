...
def get_opening_balance(
	...
	if filters.cost_center:
		...
		lft_rgt_values = frappe.db.get_values("Cost Center", {"name": ["in", cost_centers]}, ["name", "lft", "rgt"])
		lft_values = []
		rgt_values = []

		for name, lft, rgt in lft_rgt_values:
			lft_values.append(lft)
			rgt_values.append(rgt)
		...
		opening_balance = opening_balance.where(
			closing_balance.cost_center.isin(
				...
				.select("name")
				.where(
					(cost_center.lft.isin(lft_values)) &
					(cost_center.rgt.isin(rgt_values))
				)
			)
		)
